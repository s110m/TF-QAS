...
from torchquantum.util import (get_unitary_loss, legalize_unitary,
                                build_module_op_list)
from torchquantum.algorithm.quantumnas.super_utils import ArchSampler
...
from core.tools.quantize import PACTActivationQuantizer
...
class SuperQTrainer(Trainer): # Note: here we override a lot of methods of class Trainer. 
    def __init__(self, *, model: nn.Module, criterion: Callable,
                 optimizer: Optimizer, scheduler: Scheduler) -> None:
        self.model = model
        self.legalized_model = None
    ...
    def _before_epoch(self) -> None: # This will override this function in Trainer.
        self.model.train()
        self.arch_sampler.set_total_steps(self.steps_per_epoch *
                                          self.num_epochs)

    def _before_step(self, feed_dict: Dict[str, Any]) -> None:
        self.sample_arch = self.arch_sampler.get_uniform_sample_arch() # self.sample_arch is a vector like [2, 3, 1, 4, 2] that shows the architecture of the sample.   
        self.model.set_sample_arch(self.sample_arch) # Note that in the model we have set_sample_arch method. 
